...
struct DoodStreamExtractor: VideoSiteExtractor {
 ...
 let title = extractTitle(from: pageHtml) ?? "DoodStream Video"
 ...
 // DoodStream URLs are direct MP4s
 ...
 if let url = extractJsStringValue(pattern: "(?:download_url|downloadUrl|source|src|file|video_url)\\s*[=:]\\s*['\\\"]([^'\\\"]+)['\\\"]", in: html) {
 return url
 }
 // data-src="https://..."
