...
/// StreamTape exposes a direct MP4 URL via a JavaScript player on the page.
/// The video URL can be found by decoding the inline JS or by extracting
/// from a `sources: [{file: "..."}]` config that the player reads.
struct StreamTapeExtractor: VideoSiteExtractor {
    ...
    static func extract(fromHTML html: String, url: URL) async throws -> VideoSource {
        ...
        // StreamTape URLs are direct MP4s
    ...
    private static func findVideoUrl(in html: String, pageURL: URL) -> String? {
        ...
        // StreamTape commonly exposes hidden get_video links in the page HTML.
        if let url = extractHiddenGetVideoUrl(in: html, pageURL: pageURL) {
            return url
        }
    ...
    /// Decode p.a.c.k.e.r style eval blocks that StreamTape sometimes uses.
    ...
    private static func extractHiddenGetVideoUrl(in html: String, pageURL: URL) -> String? {
        let patterns = [
            #"id=["']ideoooolink["'][^>]*>([^<]+)<"#,
            #"id=["']captchalink["'][^>]*>([^<]+)<"#,
            #"id=["']norobotlink["'][^>]*>([^<]+)<"#
        ...
        for pattern in patterns {
            if let raw = extractJsStringValue(pattern: pattern, in: html) {
                let normalized = normalizeStreamTapeUrl(raw, pageURL: pageURL)
    ...
    private static func normalizeStreamTapeUrl(_ raw: String, pageURL: URL) -> String {
        ...
        if let resolved = URL(string: trimmed, relativeTo: pageURL)?.absoluteURL.absoluteString {
            return resolved
        }
        return trimmed
    }
